module edge_proc
  # (parameter
      ...
      c_nb_rows      = 8, // log2i(c_img_rows-1) + 1;
  ...
  (
    ...
    input  [1:0]   vfilter,  // (if filter_on): 1: vertical filter; 0 horizontal
  ...
  wire [8:0] grad_magnitude;  // 9 位防止溢出（8位结果 + 1位进位）
  wire [7:0] grad_magnitude_sat;
  
  wire [7:0] median_val;

median_3x3 median_unit (
    .clk(clk),  // 加入 clk 信号
  ...
  assign grad_magnitude = p_sobel_hor + p_sobel_ver; // 梯度幅值 = |Gx| + |Gy|
  // 饱和处理：限制到 8 位范围
  assign grad_magnitude_sat = (grad_magnitude > 255) ? 8'd255 : grad_magnitude[7:0];
...
begin
    if (filter_on == 1'b0)
        ...
        if (image_border) 
            ...
            2'b00: proc_pxl <= p_sobel_hor;  // 水平滤波
            2'b01: proc_pxl <= p_sobel_ver;  // 垂直滤波
            2'b10: proc_pxl <= grad_magnitude_sat; // 综合模式
            2'b11: proc_pxl <= median_val;  // 新增中值滤波
            default: proc_pxl <= 0;          // 默认值
        endcase
    end
end
